...
class _AttendancePageState extends State<AttendancePage> {
  ...
  void initializeAll() async {
    today = DateTime.now().toLocal().weekday == 1
        ? 'Monday'
        : DateTime.now().toLocal().weekday == 2
            ? 'Tuesday'
            : DateTime.now().toLocal().weekday == 3
                ? 'Wednesday'
                : DateTime.now().toLocal().weekday == 4
                    ? 'Thursday'
                    : DateTime.now().toLocal().weekday == 5
                        ? 'Friday'
                        : DateTime.now().toLocal().weekday == 6
                            ? 'Saturday'
                            : 'Sunday';
    selectedDay = today;
  ...
  Future<void> fetchAttendanceDataFromApi(BuildContext context) async {
    ...
    if (ip != '') {
      try {
        ...
        String regId = miscData.firstWhere((item) => item['key'] == 'regId',
            orElse: () => {})['value'];
        String pass = miscData.firstWhere((item) => item['key'] == 'pass',
            orElse: () => {})['value'];
        ...
        if (response.statusCode == 200) {
          ...
          if (data['status'] == 0) {
            await DBHelper.insertAttendanceData(data[
                'attendance']); // Assuming this method sets the state with the fetched data
          } else {
                          ...
                          series: <ChartSeries>[
                            BarSeries<AttendanceData, String>(
                              ...
                              dataLabelSettings: const DataLabelSettings(
                                ...
                                textStyle: TextStyle(
                                  // fontSize is 11 if phone is horizontal, 9 if vertical
                                  fontSize: 9,
                                  fontWeight: FontWeight.bold,
                                ),
                              ),
                    ...
                    child: SizedBox(
                      ...
                      child: Column(
                        ...
                        children: [
                                  ...
                                  Row(
                                    children: [
                                      ...
                                      Expanded(
                                        flex: 2,
                                        child: Text(
                                          (data.leavesApplicable > 0)
                                              ? data.leavesApplicable.toString()
                                              : 'N/A',
                                          textAlign: TextAlign.center,
                                      ...
                                      Expanded(
                                        flex: 2,
                                        child: Text(
                                          (data.mustAttendClasses != 999999)
                                              ? data.mustAttendClasses
                                                  .toString()
                                              : 'N/A',
                                          textAlign: TextAlign.center,
                                          style: const TextStyle(fontSize: 13),
                          ...
                          const Text(
                              '*Safe means that you are above 75% attendance',
                              style: TextStyle(
                                  fontSize: 12,
                                  fontStyle: FontStyle.italic,
                                  color: Colors.grey),
                              textAlign: TextAlign.center),
                          const Text('*A session has a duration of 30 minutes',
                              style: TextStyle(
                                  fontSize: 12,
                                  fontStyle: FontStyle.italic,
                                  color: Colors.grey),
                              textAlign: TextAlign.center),
                          const Text(
                              '*Leaves Applicable represents the number of sessions you can miss without falling below 75%',
                              style: TextStyle(
                                  fontSize: 12,
                                  fontStyle: FontStyle.italic,
                                  color: Colors.grey),
                              textAlign: TextAlign.center),
                          const Text(
                              '*Must Attend represents the number of sessions you must attend to reach 75%',
                              style: TextStyle(
                                  fontSize: 12,
                                  fontStyle: FontStyle.italic,
                                  color: Colors.grey),
                              textAlign: TextAlign.center),
                          const Text(
                              '*N/A means that you have already fallen below 75% and cannot reach it',
                              style: TextStyle(
                                  fontSize: 12,
                                  fontStyle: FontStyle.italic,
                                  color: Colors.grey),
  ...
  Widget showClassesScheduledForDay(String dayName) {
      ...
      if (holiday['date'] == DateFormat('yyyy-MM-dd').format(now) && dayName == today) {
        return Column(
          children: [
            ...
            const Image(
              image: AssetImage('images/holiday_.png'),
              width: 350,
            ),
            Text(
              "ITS $holidayName",
    ...
    if (dayName == 'Saturday' || dayName == 'Sunday') {
      return Column(children: [
        ...
        const Image(
          image: AssetImage('images/weekend_.jpg'),
          width: 500,
        ),
        const Text('..ITS WEEKEND..',
            style: TextStyle(
                fontSize: 20,
                color: Colors.black54,
                fontWeight: FontWeight.bold),
    ...
    if (dayName != today) {
        ...
        return Column(
          children: List.generate(
            ...
            (i) => ClassItem(
              ...
              classTime:
                  '${DateFormat('hh:mm a').format(timetableDataList[dayNumber - 1][i].startTime)} - ${DateFormat('hh:mm a').format(timetableDataList[dayNumber - 1][i].endTime)}',
      ...
      final filteredTimetableDataList = timetableDataList[dayNumber - 1]
          .where((classData) => classData.startTime.isAfter(currentTime));
      if (filteredTimetableDataList.isNotEmpty) {
        return Column(
          children: filteredTimetableDataList
              .map((classData) => ClassItem(
                    className: classData.subject,
                    classTime:
                        '${DateFormat('hh:mm a').format(classData.startTime)} - ${DateFormat('hh:mm a').format(classData.endTime)}',
        ...
        return Column(
          children: [
            ...
            const Image(
              image: AssetImage('images/doneforday.png'),
              width: 500,
            ),
            const Text('..DONE FOR THE DAY..',
                style: TextStyle(
                    fontSize: 20,
                    color: Colors.black54,
                    fontWeight: FontWeight.w500),
  ...
  Future<void> _updateClassStatus() async {
    ...
    if (today == 'Saturday' || today == 'Sunday') {
      //|| holidayDates.any((holidayDate) => today == holidayDate)) {
      isClassRunning = false;
      return;
    }
    if (now.isBefore(
            DateTime.parse("${now.toString().split(" ")[0]} 08:45:00")) &&
        now.isAfter(
            DateTime.parse("${now.toString().split(" ")[0]} 17:40:00"))) {
      // Before 8:45 AM
      isClassRunning = false;
      return;
    }
    ...
    for (int i = 0; i < timetableDataList[now.weekday - 1].length; i++) {
      if (now.isAfter(timetableDataList[now.weekday - 1][i].startTime) &&
        ...
        if (i + 1 < timetableDataList[now.weekday - 1].length &&
          ...
          int diff = timetableDataList[now.weekday - 1][i + 1]
              .startTime
              .difference(now)
              .inMinutes;
          if (diff <= 15 && diff >= 0) {
        ...
        int diff = timetableDataList[now.weekday - 1][i]
            .startTime
            .difference(now)
            .inMinutes;
        if (diff <= 15 && diff >= 0) {
  ...
  Future<void> _updateIfNewDate(BuildContext context) async {
    final lastUpdated = miscData.firstWhere(
        ...
        child: TextButton(
          ...
          style: ButtonStyle(
            backgroundColor: MaterialStateProperty.all(
              isSelected
                  ? Colors.grey[350]
                  : Colors.grey[850], // Background color
            ),
          ),
          child: Text(
